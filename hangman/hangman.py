...
'''
Робота Толстухи Надії.
Робила з:Бурлаченко Яна, Юдін Гліб, Тимошенко Дар'я, Чапанськи Марк
'''
...
'''.format(len(secret_word)))
        ...
        print ("Available letters: ", get_available_letters(letters_guessed))
            ...
            if warnings_left > 0:
                warnings_left -= 1
                print("You can't use this letter.You have {0} warnings left. {1}".format(warnings_left, get_guessed_word(secret_word, letters_guessed) ))
            if warnings_left == 0:
                guesses_left -= 1
...
You have 3 warnings left.'''.format(len(secret_word)))
    while (guesses_left > 0) and ( not is_word_guessed(secret_word, letters_guessed)):
        print('''--------------------
You have {0} guesses left.'''.format(guesses_left))
        print("Available letters:", get_available_letters(letters_guessed))
        letter = input("Please guess a letter:").lower()
        if (not letter.isalpha() and letter != "*") or (letter in letters_guessed and letter != "*"):
            if warnings_left > 0:
                warnings_left -= 1
                print("You can't use this letter.You have {0} warnings left.".format(warnings_left))
            if warnings_left == 0:
                guesses_left -= 1
                print("You lose guess.")
            else:
